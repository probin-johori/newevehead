...
import { ChatCircle, PaperPlaneRight, PencilSimple, Trash, X, CaretRight, Flag } from "@phosphor-icons/react";
...
export default function TasksPage() {
  ...
  return (
    <div className="p-6 max-w-[960px]">
      ...
      <p className="text-sm text-muted-foreground mb-5">{visibleTasks.length} tasks</p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border">
                  ...
                  <th className="px-4 py-2.5 text-left text-[12px] font-semibold uppercase tracking-wider text-muted-foreground">Due</th>
                  <th className="w-8"></th>
                </tr>
              </thead>
              <tbody>
                {deptTasks.map(t => {
                  ...
                  return (
                    <tr key={t.id} onClick={() => setSelectedTask(t.id)} className="border-b border-border last:border-0 cursor-pointer hover:bg-secondary/30 transition-colors">
            ...
            <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl bg-card border border-border shadow-[0_8px_40px_rgba(0,0,0,0.12)] p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4 border-t border-border pt-4 text-sm">
                <div>
                  ...
                  <button onClick={() => { setSelectedTask(null); navigate(`/events/${task.event_id}`); }} className="text-sm font-medium hover:text-blue-600 transition-colors">{getEvent(task.event_id)?.name}</button>
                ...
                <div>
                  <p className="text-[12px] font-semibold uppercase tracking-wider text-muted-foreground mb-1">Subtasks</p>
                          ...
                          {editingComment === c.id ? (
                            <div className="flex gap-2 mt-1">
                              <input value={editBody} onChange={e => setEditBody(e.target.value)} onKeyDown={e => e.key === "Enter" && handleEditComment(c.id)}
                                className="flex-1 rounded-lg border border-border bg-secondary px-3 py-1.5 text-sm focus:outline-none" />
                ...
                <div className="flex gap-3 mt-4">
                  ...
                  <div className="flex-1 flex gap-2">
                    <input value={newComment} onChange={e => setNewComment(e.target.value)} onKeyDown={e => e.key === "Enter" && handleSubmitComment()}
                      placeholder="Write a comment..." className="flex-1 rounded-lg border border-border bg-secondary px-3 py-2 text-sm placeholder:text-muted-foreground focus:outline-none" />
